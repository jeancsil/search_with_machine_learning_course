...
class DataPrepper:
    ...
    def __log_ltr_query_features(self, query_id, key, query_doc_ids, click_prior_query, no_results, terms_field="_id"):
        ...
        response = self.opensearch.search(log_query)
        ...
        if response and response["hits"]["hits"] and len(response["hits"]["hits"]) > 0:
            ...
            for num, hit in enumerate(hits):
                ...
                feature_results["doc_id"] = int(hit["_id"])  # capture the doc id so we can join later
                feature_results["sku"] = int(hit['_source']['sku'][0])
                feature_results["query_id"] = int(query_id)  # ^^^

                for feat_idx, feature in enumerate(features):
                    fname = feature.get("name")
                    fval = feature.get("value", 0)
                    fvals = feature_results.get(fname)
                    if fvals is None:
                        fvals = []
                        feature_results[fname] = fvals
                    fvals.append(fval)
